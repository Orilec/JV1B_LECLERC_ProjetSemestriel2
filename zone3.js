class zone3 extends Phaser.Scene {
    constructor() {
        super("zone3");
    ...
    preload() {
        ...
        this.load.image("back4", "assets/back_zone3.png");
    ...
    create() {
        this.physics.world.setBounds(0, 0, 12800, 11520);

        this.add.tileSprite(6400, 5760, 12800, 11520, 'back4');
        ...
        const map5 = this.add.tilemap("map5");

        const tileset2 = map5.addTilesetImage(
            "tileset2", "tileset2"
        );

        const plateforms = map5.createLayer(
            "platforms",
            tileset2
        );
        ...
        // const portail = map5.createLayer(
        //     "portail",
        //     tileset2
        // );
        // portail.setCollisionByProperty({ portail: true });

        const ennemiesColliders = map5.createLayer(
            "ennemies_colliders",
            tileset2
        );
        ...
        // if (!this.ePartition) {
        //     map5.getObjectLayer('partition').objects.forEach((partition) => {
        ...
        map5.getObjectLayer('enemies').objects.forEach((enemy) => {
        ...
        map5.getObjectLayer('regen').objects.forEach((regen) => {
        ...
        // map5.getObjectLayer('flying_enemies').objects.forEach((enemy) => {
        ...
        // map5.getObjectLayer('static_enemies').objects.forEach((enemy) => {
        ...
        map5.getObjectLayer('seeds').objects.forEach((seedBag) => {
        ...
        const props = map5.createLayer(
            "props",
            tileset2
        );
        this.petitSinge = this.physics.add.sprite(10200, 10900, 'petitSinge').setScale(2).setPushable(false);
        this.petitSinge.body.setAllowGravity(false);

        this.player = this.physics.add.sprite(4000, 200, 'perso');
        this.player.setScale(1.1);
        this.player.setSize(180, 490);
        this.player.setCollideWorldBounds(true);
        this.cameras.main.setBounds(0, 0, 12800, 11520);
        ...
        this.petitSingeHitbox = this.physics.add.sprite(10200, 10900, 'song').setScale(4).setPushable(false);
        ...
        this.physics.add.collider(this.carnivorous, this.wood);
        ...
        this.stepDialogue = 0;
        this.dialoguePetitSinge = ["Bravo, tu nous as tous retrouvés! \n On va pouvoir soigner Symitriel !", "Nous nous souviendrons toujours \n de ta bravoure. \n Merci de nous avoir aidé."]
    ...
    update(time, delta) {
        ...
        if (this.hp <= 0) {
            this.hp = 0;
            this.scene.start("zone3", { seedCount: this.seedCount, intro: this.intro, playerY: this.player.y, aPartition: this.aPartition, zPartition: this.zPartition, ePartition: this.ePartition, hp: 3 });
        }
        ...
        if (this.cursors2.Z.isDown && this.player.body.onFloor() && !this.scene.scene.isPlaying && !this.scene.scene.activeDialogue) {
            this.flowerRiff.play();
            this.scene.scene.plantType = "flower"
            this.playSong()
        }

        if (this.cursors2.A.isDown && this.player.body.onFloor() && !this.scene.scene.isPlaying && !this.scene.scene.activeDialogue) {
        ...
        if (this.cursors2.E.isDown && this.player.body.onFloor() && !this.scene.scene.isPlaying && !this.scene.scene.activeDialogue) {
            this.ivyRiff.play();
